...
**Quiet rehydrate (D10):** When [`QUIET_TAB_REHYDRATE.md`](QUIET_TAB_REHYDRATE.md) Tier 1–2 applies (one saved card, or last-active with toggle on), shell bootstrap copies wallet control into this tab **before** presence runs — cross-tab notices for that profile should not appear.
