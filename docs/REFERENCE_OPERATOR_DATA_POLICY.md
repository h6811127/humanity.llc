...
- **Witness scarcity (device-local ceiling):** library sunset passes also use a **browser-only** `localStorage` marker after one successful scarcity contribute per `(season_id, object_id, UTC day)` — see [`CITY_GAME_AUTONOMOUS_V1.md`](CITY_GAME_AUTONOMOUS_V1.md) § Witness scarcity. Not sent to the server; clearing site data resets it.
