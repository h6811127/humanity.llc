...
| 4a | Enable hosted flag in wrangler | **✅ Applied in repo** — `HOSTED_STEWARD_ENABLED=1` in `worker/wrangler.toml`; script remains `npm run hosted:rollout:step4a` / `-- --apply` |
| 4b | Deploy + verify production | **Next** — `npm run hosted:rollout:step4 -- --deploy` · `npm run hosted:rollout:step4 -- --smoke` (health + hosted routes on; local `API_ORIGIN=http://127.0.0.1:8787` after 4a) · verify `npm run hosted:rollout:step4 -- --verify` · `OPERATOR_AUDIT_TOKEN=... npm run hosted:rollout:step4 -- --verify` |
