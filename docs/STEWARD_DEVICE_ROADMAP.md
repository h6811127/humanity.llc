...
| **2** | **Production rollout** — D1 migrations → deploy flag off → secrets → flag on → regression | Ops / eng with Cloudflare auth | `hosted:rollout:step1` → `step6` · [`HOSTED_TIER_IMPLEMENTATION_EPICS.md`](HOSTED_TIER_IMPLEMENTATION_EPICS.md) § Production rollout — step 4b tooling + CI verify in `deploy-worker.yml` · **next (ops):** production deploy passes verify · **next (eng):** step 5a · 5b · step 6 regression |
