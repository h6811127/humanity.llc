...
| **2** | **Production rollout** — D1 migrations → deploy flag off → secrets → flag on → regression | Ops / eng with Cloudflare auth | `hosted:rollout:step1` → `step6` · step 4b–6 + 5a preflight ✅ · **next (ops):** step 5a CF pin · step 5b `--verify` · `step6 -- --verify` |
