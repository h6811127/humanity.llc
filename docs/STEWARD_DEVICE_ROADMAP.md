...
| **2** | **Production rollout** — D1 migrations → deploy flag off → secrets → flag on → regression | Ops / eng with Cloudflare auth | `hosted:rollout:step1` → `step6` · [`HOSTED_TIER_IMPLEMENTATION_EPICS.md`](HOSTED_TIER_IMPLEMENTATION_EPICS.md) § Production rollout — **step 4a ✅** (flag on in wrangler); **next:** step 4b deploy + smoke |
...
| **5** | **Child object UI** under one root (beyond `print_artifact` bridge) | **Create + update shipped** — `/created/` status plate register, list, publish status-line updates; **child scan QR next** | [`ROOT_CARD_AND_CHILD_OBJECTS.md`](ROOT_CARD_AND_CHILD_OBJECTS.md) · `created-child-object.mjs` |
