...
| **2** | **Production rollout** — D1 migrations → deploy flag off → secrets → flag on → regression | Ops / eng with Cloudflare auth | `hosted:rollout:step1` → `step6` · step 4b + 5b tooling ✅ · **next (ops):** production verify · step 5a · step 5b `--verify` · **next (eng):** step 6 regression |
