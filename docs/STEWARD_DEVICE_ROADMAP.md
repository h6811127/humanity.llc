...
Use this page when you need **one map** of steward-facing device work. Detailed rules stay in the linked docs; engineering invariants: [`SYSTEM_INVARIANTS.md`](SYSTEM_INVARIANTS.md). Doc policy: [`DOC_MAINTENANCE.md`](DOC_MAINTENANCE.md).
...
| **2** | **Production rollout** — D1 migrations → deploy flag off → secrets → flag on → regression | Ops / eng with Cloudflare auth | `hosted:rollout:step1` → `step6` · **5a ✅ · 5b preflight ✅ · 6 preflight + E2E ✅ · 4b production smoke ✅** · **remaining:** `OPERATOR_AUDIT_TOKEN=… npm run hosted:rollout:step5b -- --verify` |
| **3** | **Verify hosted path** — entitlements probe, session link, billing return E2E, SSE policy, free-tier regression | **Shipped** — run before/after rollout | `npm run hosted:rollout:verify-path` · `npm run e2e:hosted-tier-billing-return` · optional `--e2e` · [`HOSTED_TIER_G0_READINESS.md`](HOSTED_TIER_G0_READINESS.md) |
...
| **PWA install** | Phases 1–10 + **P1–P4 scan handoff** (same-tab standalone, return banner, install deferral) | Manual P1-PWA-N on device | [`PWA_INSTALL.md`](PWA_INSTALL.md) · [`PWA_STANDALONE_EXTERNAL_NAVIGATION.md`](PWA_STANDALONE_EXTERNAL_NAVIGATION.md) · **H-006** · **H-007 closed** |
...
| **Smooth mode (low-end mobile)** | Standard shell + S1–S12 mitigations | **Phase 0 complete 3/3** · Phase 1 deferred · **RC-18** Nord cold hub | [`DEVICE_LITE_MOBILE_PLAN.md`](DEVICE_LITE_MOBILE_PLAN.md) · [`DEVICE_SMOOTH_MODE_PHASE0_GATE.md`](DEVICE_SMOOTH_MODE_PHASE0_GATE.md) · [`SHELL_PAGE_LOAD_CONTENT_FLASH_INVESTIGATION.md`](SHELL_PAGE_LOAD_CONTENT_FLASH_INVESTIGATION.md) § RC-18 |
