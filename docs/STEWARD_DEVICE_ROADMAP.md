...
| **2** | **Production rollout** — D1 migrations → deploy flag off → secrets → flag on → regression | Ops / eng with Cloudflare auth | `hosted:rollout:step1` → `step6` · [`HOSTED_TIER_IMPLEMENTATION_EPICS.md`](HOSTED_TIER_IMPLEMENTATION_EPICS.md) § Production rollout — **step 4b tooling ✅** (`--preflight`, `--local-smoke`) · **next (ops):** `hosted:rollout:step4b -- --deploy` / `--smoke` / `--verify` · **next (eng):** step 5a CF dashboard · 5b CI · step 6 regression |
