...
**Glitch hoodie (launch — Tier 1, not Tier 0 inventory):** use `personalize.products[]` with target id `glitch_hoodie_v1` — artifact intent → paid webhook → Printify per-order mint. **Do not** use `TIER0_SHOPIFY_INVENTORY_VARIANT_IDS` or `tier0_glitch_hoodie_v1` shared-batch path. See [`MERCH_PRODUCT_COPY.md`](MERCH_PRODUCT_COPY.md) · [`COMPANY_MERCH_AND_COMMUNITY_CAMPAIGN.md`](COMPANY_MERCH_AND_COMMUNITY_CAMPAIGN.md) § Glitch launch decision. Post-purchase: `https://humanity.llc/shop/thanks/?hc_ref=customize_glitch`.
