...
| Merch QR calendar expiry | Founding physical artifacts (`scope: print_artifact`, Tier 0 batch) do **not** use calendar `expires_at`; URLs always resolve; owners revoke or disable. Timed expiry only for explicit event-ticket SKUs. | [`MERCH_QR_LIFECYCLE_POLICY.md`](MERCH_QR_LIFECYCLE_POLICY.md), Technical Standards | Fulfillment mint must set `expires_at: null`; scan UI must not show “QR expires” on artifact scans. |
