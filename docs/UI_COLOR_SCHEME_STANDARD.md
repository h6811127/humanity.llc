...
| Hub + inbox bottom sheets | `.device-hub.device-hub--sheet`, `.device-inbox-sheet`, sheet chrome (handle/close/title/list) | Hub sheet: **medium frosted** `--surface-popover-bg-glass` + `backdrop-filter` (see [`HUB_CARD_3D_AND_SHEET_GLASS.md`](HUB_CARD_3D_AND_SHEET_GLASS.md)); inbox remains opaque `--surface-popover-bg` until parity step. Row copy on popover fg tokens. |
