...
| Hub + inbox bottom sheets | `.device-hub.device-hub--sheet`, `.device-inbox-sheet`, sheet chrome (handle/close/title/list) | **Medium frosted** `--surface-popover-bg-glass` + `backdrop-filter: var(--shell-blur)` on hub and inbox (see [`HUB_CARD_3D_AND_SHEET_GLASS.md`](HUB_CARD_3D_AND_SHEET_GLASS.md) Steps 2–6). Row copy on popover fg tokens. |
