...
| Section order (shipped) | Hero → **Browse by need** shelves → search → **`#landing-places`** (Places near me + Sort near me) → **Boards & seasons** → **`#landing-live-object-carriers`** → **Start with one live object** → trust disclosures → device settings. Spec: [`DISCOVERY_PROJECTION.md`](DISCOVERY_PROJECTION.md) § WS-DISCOVER-P5. |
...
| Lost-item offer dismiss scope | Owner `POST …/offer/owner` dismiss must update only rows matching `(offer_id, parent_profile_id, object_id)`. Finder 201 responses include `offer_id`; an unscoped dismiss lets any steward erase another relay's pending message. |
| One active item-scoped QR | At most one `status=active` QR per `(profile_id, object_id)` for `child_object` and per `(profile_id, print_artifact_id)` for `print_artifact` — enforced by partial unique indexes (parity with card-scoped `idx_qr_one_active_card_scope`). Check-then-insert alone races under concurrent `issue-qr`. |
| Update CAS on steward writes | Card update, child-object update/revoke, and game-update must compare-and-swap on the stored `updated_at` (reject `UPDATE_CONFLICT` / freshness errors). Stale writers must not rewind timestamps or clobber newer manifesto / game_node truth. |

**Regression:** `npm run worker:test -- worker/tests/scan.test.ts worker/tests/scan-freshness-banner.test.ts worker/tests/scan-revoke-truth-contract.test.ts worker/tests/scan-live-truth-core.test.ts worker/tests/scan-cache-purge.test.ts worker/tests/update-card.test.ts worker/tests/live-object-staleness-contract.test.ts worker/tests/live-object-succession-spec.test.ts worker/tests/lost-item-offer.test.ts` · E2E: `npm run e2e:scan-revoke-freshness`
