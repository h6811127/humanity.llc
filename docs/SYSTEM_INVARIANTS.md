...
| Child object id scope | `child_objects` primary key is `(parent_profile_id, object_id)`. Deterministic catalog IDs (`obj_cr_node_*`, `obj_<season>_node_*`) must not let one steward block another’s create. Lookups and unlock side effects are parent-scoped (season root when bound). |
