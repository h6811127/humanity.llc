...
| Personalized paid qty | Shopify `orders/paid` must hold (`ARTIFACT_INTENT_QUANTITY_MISMATCH`) when summed line quantities for an `artifact_intent_id` ≠ `artifact_intents.quantity` / `planned_item_qr_ids.length`. Never auto-fulfill a personalized intent from a mismatched paid cart qty. |
