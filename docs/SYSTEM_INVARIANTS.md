...
| Tier 0 batch quantity | Shopify paid line quantity for Tier 0 batch variants must persist on `print_orders.quantity` and drive Printify `line_items[].quantity`. Empty `planned_item_qr_ids` (shared campaign QR) must not force qty `1` when the buyer paid for N. |
