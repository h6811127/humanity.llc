...
| Game node season binding | `game_node` child-object create/update documents must use the same `season_id` resolved from the parent profile’s season root; the resolver rejects cross-season nodes before insert so scans/contributes cannot bind to the wrong graph. Revoke remains available for cleanup. |
