...
| Hybrid `device_unlock` | `hc_wallet` rows may store `wrapped_owner_key` only — never plaintext `owner_private_key_b58` in `localStorage` **when a usable wrap exists**. Unlock via WebAuthn → populate `hc_created` per tab. Quiet rehydrate **must not** silently copy wrapped rows (C2). Layer 2 copy: **Unlock to manage**, not restore jargon. Cross-device (C4 / K11): recovery import **strips stale wrap** and marks re-enroll pending; backup import restores owner key; wallet sync / `/created/` autosave **must not strip** plaintext while `device_unlock` has no wrap (or re-enroll is pending); Manage offers passkey re-enroll which wraps again. |
