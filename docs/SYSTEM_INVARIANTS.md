...
| Time policy timezone | `time_policy.timezone` is an IANA zone (or `UTC`). Create/update reject unknown ids. Public scan must stay HTTP 200 if a stored document has a typo — skip weekly-hour evaluation, keep absolute windows (`valid_from` / `valid_until` / dormant / grace). |
