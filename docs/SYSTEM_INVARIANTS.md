...
| Invariant | Detail |
|-----------|--------|
| Model | **Live world index** (physical internet front door) — discovery-first browse of public programmable objects / places / boards **before** scan. Not a marketing homepage, shop grid, or **beachhead-city product identity**. Cedar Rapids (and other regions) are **catalog density**, not the noun of `/`. Spec: [`PRODUCT_POSITIONING_AND_LOOP_STRATEGY.md`](PRODUCT_POSITIONING_AND_LOOP_STRATEGY.md) § Front door strategy · [`DISCOVERY_PROJECTION.md`](DISCOVERY_PROJECTION.md). |
| Hero H1 (shipped) | **“Check what's true right now before you knock, pick up, or show up.”** — eyebrow **Public programmable objects**; trust chips below (No account · Community-run boards · No scan surveillance). Subline: live public layer / world-index voice (not beachhead-city identity). |
| Hook (secondary) | **“The sticker stays — the status changes.”** on the create footnote (`#landing-start-object-cta`), not the hero H1. |
| Section order (shipped) | Hero → **Browse by need** shelves → search → **`#landing-places`** (Places near me + region picker + Sort near me) → **Boards & seasons** → **`#landing-live-object-carriers`** → **`#landing-try-live-object`** (company demo plate; soft-hidden if pointer missing) → **Start with one live object** → trust disclosures → device settings. Spec: [`DISCOVERY_PROJECTION.md`](DISCOVERY_PROJECTION.md) § WS-DISCOVER-P5 · DEMO. |
...
| Forbidden | “Live state on real objects” as hero H1; hero-only **`landing-hero-btn-primary`** Create CTA; “One use · status plate”; **leading with lost-item relay as company identity**; product carousel on `/`; full shop embed or checkout on `/`; framing `/` as a **single-city portal** or **game-company** homepage (city game / merch remain wedges on the same rails). **`#landing-try-live-object`** is allowed as a **secondary** company-hosted demo (not a hero door, not guest-editable). |
| Sparse honesty | Fail-closed on nearness / fullness / “live now” counts until true; fail-open on a real small catalog or honest empty — never pad pins or imply one region is the whole physical internet. Empty region → escape via region picker / All regions; geo denied → listed order + honest status; load fail → recovery copy (DENSE · [`DISCOVERY_PROJECTION.md`](DISCOVERY_PROJECTION.md) § P5). |
