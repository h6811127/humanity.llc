...
| Relationship edge identity | `edge_id` is stable within its steward graph and is stored/looked up by `(steward_profile_id, edge_id)`. Deterministic public edge IDs must never let another steward preempt issue or revoke operations. |
