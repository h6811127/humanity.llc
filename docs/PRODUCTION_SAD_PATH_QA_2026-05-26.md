...
| ID | Scenario | Tool |
|----|----------|------|
| S1 | Incognito: `/` → create → close tab without save → reopen `/` | **Automated (simulated):** `e2e/production-sad-path-created.spec.ts` (cleared storage → `/`) · full incognito still manual |
| S2 | Incognito: bogus `/created/?profile_id=` | **Automated:** `e2e/production-sad-path-created.spec.ts` |
| S3 | Two tabs: create in B, hub in A | **Automated:** `e2e/device-cross-tab-keys.spec.ts` · `DEVICE_OS_QA.md` P0-1 |
