...
3. **Landing de-explain**  -  shorter hero + compact framing on `/` (**v2 shipped:** studio example uses, **Design choices**, **Clear limits**, and **Documentation** default closed in `<details>`; below-fold tutorial blocks use `content-visibility: auto`; status-dot pulse and chrome blur pause while scrolling via `shell-is-scrolling` in `device-shell-chrome.mjs`).
