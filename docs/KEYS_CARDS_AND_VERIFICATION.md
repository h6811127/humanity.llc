...
**Related:** [`PRODUCT_LANGUAGE_STRATEGY.md`](PRODUCT_LANGUAGE_STRATEGY.md) (canonical plain-vs-precise policy) · [`OWNERSHIP_AND_CONTROL_MODEL.md`](OWNERSHIP_AND_CONTROL_MODEL.md) (Layer 2 product language vs this Layer 1 reference) · [`SAFARI_KEYS_CUSTODY.md`](SAFARI_KEYS_CUSTODY.md) (WebKit session vs wallet) · [`ROOT_CARD_AND_CHILD_OBJECTS.md`](ROOT_CARD_AND_CHILD_OBJECTS.md) · [`VOUCH_TRUST_POSITIONING.md`](VOUCH_TRUST_POSITIONING.md) · [`M6_VOUCHING_DESIGN.md`](M6_VOUCHING_DESIGN.md) · [`DEVICE_HUB_AND_LOCAL_SEARCH.md`](DEVICE_HUB_AND_LOCAL_SEARCH.md) · [`KEYS_CUSTODY_AND_NOTIFICATION_IMPROVEMENT_PLAN.md`](KEYS_CUSTODY_AND_NOTIFICATION_IMPROVEMENT_PLAN.md), [`V1_PRODUCT_TRUST_MODEL.md`](V1_PRODUCT_TRUST_MODEL.md)

**Layer note:** This doc is **Layer 1** (engineering reference). Default UI copy should follow [`OWNERSHIP_AND_CONTROL_MODEL.md`](OWNERSHIP_AND_CONTROL_MODEL.md) terminology map and [`PRODUCT_LANGUAGE_STRATEGY.md`](PRODUCT_LANGUAGE_STRATEGY.md); keep this doc precise for builders and support depth.
...
| **Shell performance** | `hc_wallet_summary` on status/count, collapsed hub preview, large expanded hub summary, incremental summary-window paths, and **viewport scroll-sync** (visible-row expansion + sentinel/scroll load); display-safe rows and poll hot paths (S8–S8b); signing hydrates via `findWalletEntryById` on full rows and on action for large expanded summary rows; **`hc_wallet_network_cache`** capped at **20** rows (S6); full-row hub/wallet DOM caps at **15** / **40** (S10–S11). | Optional later: true virtual scroll recycle on `/wallet/` for N ≫ 40 — [`SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md`](SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md). |
| **Multi-tab** | Extra tabs with `hc_created` add cross-tab presence churn (`hc_tab_keys_presence`, capped at **20 tab rows**). That is separate from wallet size but **feels worse with many saved roots + many tabs** (storage events → chrome refresh). Resolver tab sync: [`DEVICE_TAB_RESOLVER_SYNC.md`](DEVICE_TAB_RESOLVER_SYNC.md). | Lag and false-positive inbox paths; unified hub custody panel — [`KEYS_CUSTODY_AND_NOTIFICATION_IMPROVEMENT_PLAN.md`](KEYS_CUSTODY_AND_NOTIFICATION_IMPROVEMENT_PLAN.md) · [`CROSS_TAB_KEYS_NOTIFICATION_SYSTEM.md`](CROSS_TAB_KEYS_NOTIFICATION_SYSTEM.md). |

**Engineering status:** S6–S11, `hc_wallet_summary`, large expanded hub summary rows, incremental summary-window paths, viewport scroll-sync for expanded hub summary rows, O2 hot-route rate limits, and large-wallet presence debounce shipped (2026-05-28). Re-test multi-tab at very large N — [`DEVICE_OS_REQUEST_BUDGET.md`](DEVICE_OS_REQUEST_BUDGET.md) § Open issues.
...
| **`/wallet/` saved card row** | **Title** (label or `@handle`); **identity line** (`Object type · Steward / Vouched Human / Registered`); **one status line** (e.g. **Reachable · checked 2m ago**, **QR revoked**, **Disabled on network**). **General roots:** nested child rows under the root (`public_label`, `{Status plate \| Lost item} · under @handle`, no trust shield on child) — shipped step 13; see [`HUB_CARD_ROW_UX.md`](HUB_CARD_ROW_UX.md) Phase 6. Flat pilot cards remain single rows. Technical fields in collapsed **Details**. |
