...
| **Shell performance** | Hub/inbox chrome uses `hc_wallet_summary` and display-safe rows (S8–S9); poll paths avoid full-array copies (S8b). **`hc_wallet_network_cache`** capped at **20** rows (S6). Hub/wallet DOM caps at 15/40 rows (S10–S11). | Residual lag at very large N or many tabs — see [`SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md`](SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md). |
...
**Engineering status:** Shell perf S6–S11, `hc_wallet_summary`, resolver hot-route rate limits (O2), and large-wallet presence debounce shipped (2026-05-28). Re-test multi-tab at very large N — see [`DEVICE_OS_REQUEST_BUDGET.md`](DEVICE_OS_REQUEST_BUDGET.md) § Open issues.
