...
| **Shell performance** | Status/count chrome now uses a lightweight `hc_wallet_summary` index, but full hub/inbox row rendering still hydrates `hc_wallet`; `sessionStorage.hc_wallet_network_cache` is capped at 20 fresh rows per session. | Hub, glance, and inbox still need lazy row hydration before large root wallets are supported; see [`SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md`](SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md) § persistence table. |
...
**Engineering status:** Partially mitigated for status/count hot paths by `hc_wallet_summary`; large-wallet hub/inbox row hydration remains unsupported.
