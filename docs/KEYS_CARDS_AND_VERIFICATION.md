...
| **Shell performance** | Status/count chrome, hub glance, cross-tab saved-profile checks, and card-disabled inbox reads now use a lightweight `hc_wallet_summary` index; full expanded hub row rendering still hydrates `hc_wallet`; `sessionStorage.hc_wallet_network_cache` is capped at 20 fresh rows per session. | Expanded hub rows still need lazy hydration before large root wallets are supported; see [`SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md`](SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md) § persistence table. |
...
**Engineering status:** Partially mitigated for status/count, hub glance, cross-tab, and card-disabled inbox hot paths by `hc_wallet_summary`; expanded large-wallet hub row hydration remains unsupported.
