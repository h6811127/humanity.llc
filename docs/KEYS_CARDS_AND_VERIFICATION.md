...
| **Shell performance** | Status/count chrome, compact hub/inbox rows, cross-tab saved-profile checks, card-disabled inbox reads, collapsed hub previews, large expanded hub summary rows, and incremental summary-row windows now use a lightweight `hc_wallet_summary` index; full row data hydrates on action for large expanded hubs; `sessionStorage.hc_wallet_network_cache` is capped at 20 fresh rows per session. | Viewport-based DOM virtualization is still needed before very large root wallets are supported; see [`SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md`](SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md) § persistence table. |
...
**Engineering status:** Partially mitigated for status/count, compact hub/inbox, cross-tab, card-disabled inbox, collapsed preview, large expanded hub summary, and incremental summary-window hot paths by `hc_wallet_summary`; viewport-based virtualization remains unsupported.
