...
| **Shell performance** | Status/count chrome, compact hub/inbox rows, cross-tab saved-profile checks, card-disabled inbox reads, collapsed hub previews, and large expanded hub summary rows now use a lightweight `hc_wallet_summary` index; full row data hydrates on action for large expanded hubs; `sessionStorage.hc_wallet_network_cache` is capped at 20 fresh rows per session. | True DOM virtualization is still needed before very large root wallets are supported; see [`SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md`](SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md) § persistence table. |
...
**Engineering status:** Partially mitigated for status/count, compact hub/inbox, cross-tab, card-disabled inbox, collapsed preview, and large expanded hub summary hot paths by `hc_wallet_summary`; true DOM virtualization remains unsupported.
