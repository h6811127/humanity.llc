...
| **Shell performance** | Status/count chrome, compact hub/inbox rows, cross-tab saved-profile checks, card-disabled inbox reads, and collapsed hub previews now use a lightweight `hc_wallet_summary` index; full expanded hub row rendering still hydrates `hc_wallet`; `sessionStorage.hc_wallet_network_cache` is capped at 20 fresh rows per session. | Expanded hub rows still need visible-row hydration/virtualization before large root wallets are supported; see [`SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md`](SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md) § persistence table. |
...
**Engineering status:** Partially mitigated for status/count, compact hub/inbox, cross-tab, card-disabled inbox, and collapsed hub preview hot paths by `hc_wallet_summary`; expanded large-wallet hub row virtualization remains unsupported.
