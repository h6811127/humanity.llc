...
| **Shell performance** | `hc_wallet_summary` on status/count, collapsed hub preview, large expanded hub summary, and incremental summary-window paths; display-safe rows and poll hot paths (S8–S8b); signing hydrates via `findWalletEntryById` on full rows and on action for large expanded summary rows; **`hc_wallet_network_cache`** capped at **20** rows (S6); full-row hub/wallet DOM caps at **15** / **40** (S10–S11). | Viewport-based DOM virtualization for very large expanded hubs; optional later virtual scroll on `/wallet/` for N ≫ 40 — [`SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md`](SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md). |
...
**Engineering status:** S6–S11, `hc_wallet_summary`, large expanded hub summary rows, incremental summary-window paths, O2 hot-route rate limits, and large-wallet presence debounce shipped (2026-05-28). Viewport-based virtualization for very large expanded hubs remains open. Re-test multi-tab at very large N — [`DEVICE_OS_REQUEST_BUDGET.md`](DEVICE_OS_REQUEST_BUDGET.md) § Open issues.
