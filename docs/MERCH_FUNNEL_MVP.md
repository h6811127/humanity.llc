...
| **3** | **Hosted steward production rollout** — `hosted:rollout:step*` through step 6 (secrets, flag, CF dashboard, regression) | Ops | **In progress** — steps 1–2 ✅ · step 3a smoke + `OPERATOR_AUDIT_TOKEN` verify · step 4a flag-on config ✅ · step 4b `--smoke` tooling ✅ · **next:** deploy + `--smoke` / `--verify` on production |
