...
| **5** | **Large-wallet shell performance** — bound `hc_wallet_network_cache`, avoid full-wallet parse on hub/inbox hot paths | Engineering debt | **Partial ✅** — S6 cache bound (`WALLET_NETWORK_CACHE_MAX_ENTRIES`); shell/scan status counts now use lightweight `hc_wallet_summary`; remaining: lazy row hydration for full hub/inbox rows |
