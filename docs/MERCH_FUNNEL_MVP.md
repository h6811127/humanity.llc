...
| **3** | **Hosted steward production rollout** — `hosted:rollout:step*` through step 6 (secrets, flag, CF dashboard, regression) | Ops | **In progress** — step 4b + 5b tooling ✅ · step 6 regression ✅ · **next (ops):** production deploy passes verify · step 5a CF dashboard · token-backed step 5b `--verify` |
