...
| **3** | **Hosted steward production rollout** — `hosted:rollout:step*` through step 6 (secrets, flag, CF dashboard, regression) | Ops | **In progress** — steps 1–4b tooling ✅ (`hosted:rollout:step4b -- --preflight`, `--local-smoke`) · **next (ops):** `hosted:rollout:step4b -- --deploy` / `--smoke` / `--verify` on production · **next (eng):** step 5a CF dashboard · step 5b CI · step 6 regression |
