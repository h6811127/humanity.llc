...
| **5** | **Large-wallet shell performance** — bound `hc_wallet_network_cache`, avoid full-wallet parse on hub/inbox hot paths | Engineering debt | **✅** — S6 cache bound; S8 metadata hot paths; S9 hub rows use display-safe wallet views (`listWalletDisplayEntries`) |
