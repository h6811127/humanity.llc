...
| **3** | **Hosted steward production rollout** — `hosted:rollout:step*` through step 6 (secrets, flag, CF dashboard, regression) | Ops | **In progress** — step 1 local preflight ✅ (`hosted:rollout:step1`); step 2 smoke verifies flag-off gate (`hosted:rollout:step2 -- --smoke`) |
