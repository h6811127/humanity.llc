...
| **3** | **Hosted steward production rollout** — `hosted:rollout:step*` through step 6 (secrets, flag, CF dashboard, regression) | Ops | **In progress** — step 4b tooling + CI ✅ (`hosted:rollout:step4b`, `deploy-worker.yml` post-deploy verify) · **next (ops):** production deploy passes verify · **next (eng):** step 5a CF dashboard · step 5b CI · step 6 regression |
