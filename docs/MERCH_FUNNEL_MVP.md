...
| **3** | **Hosted steward production rollout** — `hosted:rollout:step*` through step 6 (secrets, flag, CF dashboard, regression) | Ops | **In progress** — step 4b + scan smoke + `schemaReady` guard ✅ · **next (ops):** `hosted:rollout:step5 -- --verify` then `step6 -- --verify` · **next (eng):** step 5a CF dashboard |
