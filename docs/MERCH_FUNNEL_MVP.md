...
| **3** | **Hosted steward production rollout** — `hosted:rollout:step*` through step 6 (secrets, flag, CF dashboard, regression) | Ops | **In progress** — steps 1–2 ✅ · step 3a smoke + `OPERATOR_AUDIT_TOKEN` verify · **next:** step 4a (`hosted:rollout:step4a -- --apply`) → step 4b deploy + `hosted:rollout:step4 -- --smoke` / `--verify` |
