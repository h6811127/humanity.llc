...
| **3** | **Hosted steward production rollout** — `hosted:rollout:step*` through step 6 (secrets, flag, CF dashboard, regression) | Ops | **In progress** — steps 1–3a ✅ · step 4a ✅ · **next:** step 4b deploy + `hosted:rollout:step4 -- --smoke` / `--verify` on production · local preflight: `hosted:rollout:step4 -- --smoke --local --preflight` after `worker:dev` |
