...
| **3** | **Hosted steward production rollout** — `hosted:rollout:step*` through step 6 (secrets, flag, CF dashboard, regression) | Ops | **In progress** — rollout tooling ✅ (steps 4b–6 incl. `--preflight`) · **next (ops):** production verify · step 5a · step 5b `--verify` · step 6 `--verify` before announcement |
