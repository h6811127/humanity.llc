...
Every hub/inbox pass used to call `loadWallet()` and hydrate the full `hc_wallet` array. **`hc_wallet_network_cache`** is now capped at **20** fresh rows per session (S6, 2026-05-27), and shell/scan status counts read a lightweight `hc_wallet_summary` index instead of full key-bearing wallet rows on hot status paths. Remaining: lazy row hydration for full hub/inbox row rendering. See [`SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md`](SAFARI_PERFORMANCE_AND_REFRESH_INVESTIGATION.md).
...
| Date | Note |
|------|------|
| 2026-05-28 | **Large-wallet shell perf:** added `hc_wallet_summary` so status/count hot paths avoid full `hc_wallet` row hydration |
