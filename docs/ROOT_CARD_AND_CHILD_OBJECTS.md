...
14. **Create flow convergence (shipped):** `/create/` emphasizes general Humanity Card; status plate / lost item show Add-on-Live nudge when a general root with keys exists (labeled `hc-emphasis-card--info` with **Recommended path** eyebrow + paired CTA pills; E2E `e2e/create-flow-convergence.spec.ts`); legacy standalone pilot forms stay in a disclosure; landing copy updated.
