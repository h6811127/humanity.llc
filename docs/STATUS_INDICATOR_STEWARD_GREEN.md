...
**Red outline ring on the dot button:** that is `data-device-status-error` (module load failure), not custody red. A **load-error coach card** auto-opens below the dot with refresh guidance; the hub does not open until the status graph loads. See [`STATUS_DOT_LOAD_FAILURE_POSTMORTEM.md`](STATUS_DOT_LOAD_FAILURE_POSTMORTEM.md) for the 2026-05-25 regression, load-error coach card, timeline, and prevention plan.
...
1. **Console on first load** — Red errors loading `device-status.mjs` or any import mean the module aborted and the **hub** click listener was not registered. If `#top-chrome` has `data-device-status-error`, the load-error coach card should appear — not a silent dead control.
...
|----------|--------|---------|
| 1 | **`device-status.mjs` module load failure** | Dead hub on all shell pages; red outline + load-error coach card; console import/404 errors; no `dot_click` in diag log |
...
- **CI (shipped):** `e2e/device-status-dot.spec.ts` — `shell status modules are reachable`, `status bootstrap loads and records dot_click in diagnostics`, `status partial load opens hub without error coach hijack`, `total status load error shows coach card`, `bootstrap inner failure shows load-error coach card` · `worker/tests/device-status-core.test.ts` · `worker/tests/device-status-partial-load.test.ts` · `worker/tests/device-status-lazy-inbox.test.ts` (inbox + hub-sheet loaders) · `worker/tests/device-inbox-loader.test.ts`.
