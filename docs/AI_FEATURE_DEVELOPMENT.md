...
**Status:** Active — **L0–L2 shipped**; L3 P1 shipped as **deterministic-only** reader (Workers AI dropped 2026-09-08); **L3 P2 UI retired** (2026-05-27)  
...
| **L3 P1** — opt-in scan reader | **Kept — deterministic-only** (2026-09-08: Workers AI dropped; restates signed fields, no model) |
...
| **L3 Readers / orchestration** | Optional read-only layer on L0–L2 only | P1 shipped (deterministic-only); P2 UI retired |
...
See [`AI_L3_EXPLAIN_SNAPSHOT.md`](AI_L3_EXPLAIN_SNAPSHOT.md). **Step 2 done** (keep opt-in, de-emphasize AI in UI copy). **Priority 4 closed (2026-09-08): deterministic-only** — Workers AI binding removed; endpoint restates signed fields with no model.
...
| **6** | **Ephemeral state UX for Tier 1 merch** — unlock `/created/` update without revoke-first gate; Tier 1 thanks path → `#update-status`. Product moat is L0–L2, not AI. | ✅ Shipped | [`EPHEMERAL_STATE_AND_MERCH.md`](EPHEMERAL_STATE_AND_MERCH.md) |
| **7** | **Resolve Priority 4 — L3 P1 reader decision → deterministic-only** — drop the Workers AI dependency from the scan explainer; keep opt-in restatement + stable `ai` API fields; update docs + tests. | ✅ Done (2026-09-08) | [`AI_L3_EXPLAIN_SNAPSHOT.md`](AI_L3_EXPLAIN_SNAPSHOT.md) · `worker/src/resolver/ai-explain-*.ts` · `worker/wrangler.toml` · `MERCH_FUNNEL_MVP.md` § Implementation priority stack |

---

## Ephemeral state vs AI (product moat)

Physical merch value is **mutable signed state on immutable ink** — not generative copy.

| Layer | Owner | Stranger |
|-------|-------|----------|
| L0–L2 | Updates from `/created/` or hub | Reads signed snapshot on scan |
| L3 P1 | N/A (stewards write copy) | Opt-in plain-language restatement only |

See [`EPHEMERAL_STATE_AND_MERCH.md`](EPHEMERAL_STATE_AND_MERCH.md) for Tier 1 funnel step **WEAR** and accessibility without LLMs. **Priority 4 closed (2026-09-08):** L3 P1 is **deterministic-only** — keep the opt-in reader as a model-free restatement of signed fields.
...
| **Review** | ~~P1 scan reader~~ | **Closed 2026-09-08 — deterministic-only** (accessibility kept, model dropped) |
...
| [`EPHEMERAL_STATE_AND_MERCH.md`](EPHEMERAL_STATE_AND_MERCH.md) | Same ink / new meaning — Tier 1 UX |
