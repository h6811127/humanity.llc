...
| `/.well-known/hc/v1/vouch-appeals` | `POST` | Public appeal intake (receipt-only response; see below) |
...
Public routes must **not** echo operator case rows. Case status, subjects, and priority stay on `OPERATOR_AUDIT_TOKEN` case routes.

**`POST …/vouch-reports`** (201):

- Request: `kind`, `target`, `statement`, optional `contact_method`.
- Response: `{ ok, report_id, reference_code }` only. `reference_code` when `contact_method` is provided.
- The worker dedupes into `public_report` cases server-side; the JSON shape is the same whether a new case was opened or an existing open case received another report.
- Must **not** return: `case`, `case_created`, `case_id`, `status`, `priority`, `subject_profile_ids`, `subject_vouch_ids`, or other inbox fields.

**`POST …/vouch-appeals`** (201):

- Request: `case_id`, `profile_id`, `statement`, optional `contact_method`.
- Response: `{ ok, appeal_id, reference_code }` only. `reference_code` when `contact_method` is provided.
- First appeal on a `suspended` case moves the case to `appealed` server-side; follow-up appeals on an already-`appealed` case are accepted without changing status again.
- Must **not** return: `case`, `case_status_changed`, `case_id` (beyond request), `status`, `priority`, `subject_profile_ids`, or other inbox fields.
...
- Public appeal rate limit.
- Public appeal intake returns receipt fields only (no case metadata in JSON).
- Suspension overrides verification summary in scan/status.
- Appeal moves suspended case to appealed and records appeal row.
...
- Public report form at `/report/` submits without exposing case state in the response.
...
5. **Appeals**: suspended profile appeal path and case status transitions. **Shipped** — `POST …/vouch-appeals`, `/appeal/`
