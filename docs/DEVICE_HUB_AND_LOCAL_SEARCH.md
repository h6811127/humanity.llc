...
**Landing story (shipped):** Discovery hero **“Check what's true right now before you knock, pick up, or show up.”** + trust chips → **Browse by need** shelves → search → **Public live boards** → **Start with one live object** (`#landing-start-object-cta`, hook footnote *The sticker stays — the status changes.*) → trust disclosure cards → device settings. Front-door spec: [`PRODUCT_POSITIONING_AND_LOOP_STRATEGY.md`](PRODUCT_POSITIONING_AND_LOOP_STRATEGY.md) § Front door strategy. Do not revert to “Live state on real objects” or a hero-only Create CTA — `worker/tests/landing-messaging.test.ts` + `landing-copy-contract.mjs` lock discovery hero and section order. **Retired from `/`:** **`#launch-doors`** three-row chooser · **How it works** flow strip · **Ready to try it?** closing CTA · homepage pass-card demo.
