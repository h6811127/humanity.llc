...
We kept the landing as a **discovery dashboard** (hero → shelves → public boards → steward/commerce rows → trust disclosures) and **enriched** it with device hub chrome rather than replacing it with a full marketing funnel.

**Landing story (shipped):** Discovery hero **“Check what's true right now before you knock, pick up, or show up.”** + trust chips → **Browse by need** shelves → search → **`#landing-places`** (Places near me + Sort near me) → **Boards & seasons** → **`#landing-live-object-carriers`** → **Start with one live object** (`#landing-start-object-cta`, hook footnote *The sticker stays — the status changes.*) → trust disclosure cards → device settings. Front-door spec: [`PRODUCT_POSITIONING_AND_LOOP_STRATEGY.md`](PRODUCT_POSITIONING_AND_LOOP_STRATEGY.md) § Front door strategy · [`DISCOVERY_PROJECTION.md`](DISCOVERY_PROJECTION.md) § WS-DISCOVER-P5. Do not revert to “Live state on real objects” or a hero-only Create CTA — `worker/tests/landing-messaging.test.ts` + `landing-copy-contract.mjs` lock discovery hero and section order. **Retired from `/`:** **`#launch-doors`** three-row chooser · **How it works** flow strip · **Ready to try it?** closing CTA · homepage pass-card demo · **Public live boards** H2 (replaced by **Boards & seasons**).

**Landing carriers row (shipped):** **`#landing-live-object-carriers`** — static **Live object carriers** featured row after **`#public-networks-results`**, before `#landing-start-object-cta`. Hydrates from `shop-config.json` + store catalog; static HTML fallback. **See all carriers →** `/shop/`. **No** carousel. Modules: [`landing-live-object-carriers-core.mjs`](../site/js/landing-live-object-carriers-core.mjs) · [`MERCH_VISUAL_CHOREOGRAPHY.md`](MERCH_VISUAL_CHOREOGRAPHY.md) § Landing carriers row.

**On this device** is the hub sheet below the status line (inline search, no floating help pill). Device settings sit after trust disclosures on `/`.
...
**Child objects under a root (first slice shipped — step 13):** General root rows in **My cards** / expanded hub show **nested child rows** (status plate, lost-item relay) from `hc_child_objects_v1:{profile_id}`, reconciled from `GET …/objects` on hub render. Children are not separate `hc_wallet` entries. Differentiation: object-first title (`public_label`), identity line `{type} · under @handle`, no human-trust shield on the child row, **Update status** / **Open scan** on the child row. Spec: [`ROOT_CARD_AND_CHILD_OBJECTS.md`](ROOT_CARD_AND_CHILD_OBJECTS.md) § My cards and hub presentation · [`HUB_CARD_ROW_UX.md`](HUB_CARD_ROW_UX.md) Phase 6. Helpers: `site/js/hub-child-object-row-core.mjs`; render: `site/js/device-hub-ui.mjs`.
