...
**Landing story (shipped):** Hero H1 **“The sticker stays. The status changes.”** + trust chips, **Three ways in** launch doors (**Live status on something** · **Live status on you** · **Play the city game**), privacy emphasis cards, **How it works** flow strip, then framing and **Live status objects** example. Front-door spec: [`PRODUCT_POSITIONING_AND_LOOP_STRATEGY.md`](PRODUCT_POSITIONING_AND_LOOP_STRATEGY.md) § Front door strategy. Do not revert to “Live state on real objects” or a single hero Create CTA — `worker/tests/landing-messaging.test.ts` + `landing-copy-contract.mjs` lock the sticker hero and `#launch-doors`. **On this device** is the hub sheet below the status line (inline search, no floating help pill). **Shortcuts & settings** sit after the studio block so first scroll stays product narrative. Closing **Ready to try it?** CTA before documentation (`hc-emphasis-card--urgent` — [`LANDING_FINAL_CTA_EMPHASIS_CARD.md`](LANDING_FINAL_CTA_EMPHASIS_CARD.md); visual alignment with keys cards — [`HC_EMPHASIS_CARD_VISUAL_ALIGNMENT.md`](HC_EMPHASIS_CARD_VISUAL_ALIGNMENT.md)). Intro band uses a shared 22px section-title ladder for **How it works** / **Ready to try it?** Homepage pass-card demo removed; strangers see real scan pages and the case study.
