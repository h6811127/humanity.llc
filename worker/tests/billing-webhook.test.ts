...
describe("handlePostBillingWebhook", () => {
  ...
  it("keeps hosted_game_season_v1 caps when invoice.payment_failed opens grace", async () => {
    const { db, accounts, sessions } = billingDb();
    accounts.set(ACCOUNT, {
      ...accounts.get(ACCOUNT)!,
    ...
    sessions.add("season_session_should_survive_grace");
    ...
    expect(json.result).toBe("ok");
    expect(row.status).toBe("past_due");
    expect(row.plan_id).toBe("hosted_game_season_v1");
    expect(Date.parse(String(row.effective_until))).toBeGreaterThan(Date.now());
    expect(sessions.size).toBe(1);
  });
});
