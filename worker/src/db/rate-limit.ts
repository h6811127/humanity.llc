...
export async function checkVouchReportRateLimit(
  ...
  const windowIso = windowStart.toISOString();
  const bucketKey = `${VOUCH_REPORT_BUCKET_PREFIX}${ipHash}:${windowIso}`;
  ...
  const count = row?.count ?? 0;
  if (count >= VOUCH_REPORT_LIMIT_PER_HOUR) {
    const nextHour = new Date(windowStart.getTime() + 3600_000);
    return {
      allowed: false,
      retryAfterSec: Math.max(
...
export async function checkVouchAppealRateLimit(
  ...
  const windowIso = windowStart.toISOString();
  const bucketKey = `${VOUCH_APPEAL_BUCKET_PREFIX}${ipHash}:${windowIso}`;
  ...
  const count = row?.count ?? 0;
  if (count >= VOUCH_APPEAL_LIMIT_PER_HOUR) {
    const nextHour = new Date(windowStart.getTime() + 3600_000);
    return {
      allowed: false,
      retryAfterSec: Math.max(
