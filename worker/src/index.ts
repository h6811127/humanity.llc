...
import { handleGetHostedStewardOps } from "./resolver/steward-ops";
...
export default {
  ...
  async fetch(
    ...
    if (
      path === "/.well-known/hc/v1/operator/hosted-steward/ops" &&
      request.method === "GET"
    ) {
      if (!env.DB) {
        return withCors(
      ...
      const res = await handleGetHostedStewardOps(request, env, env.DB);
      return withCors(request, res);
    }
    if (
