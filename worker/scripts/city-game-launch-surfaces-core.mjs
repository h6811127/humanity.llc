 ...
 * @param {Record<string, unknown>} season
 */
export function applyRulesPageLaunchPatches(html, season) {
  const label = formatSeasonWindowLabel(season);
  if (!label) {
    throw new Error("Season window dates required for rules page launch patch.");
  }

  let out = html.replace(NOINDEX_META_RE, "\n");
  const liveHint = `<div class="research-live-banner hc-notice hc-notice--info" role="note">
          <p>
            <strong>Season live.</strong> ${label}
          </p>
          <p>
            Scan game stickers around Cedar Rapids to read public object state. No account required.
          </p>
        </div>`;

  if (!RULES_DRAFT_HINT_RE.test(out)) {
    throw new Error("Rules page missing research-dev-hint block.");
  }
  ...
  out = out.replace(
    /content="Rules draft for Cedar Rapids Season 1[^"]*"/,
    'content="Rules for Cedar Rapids Season 1 — public object state, cooperation-first play, and what scans prove without tracking players."'
  );
  return out;
}
 ...
 * @param {Record<string, unknown>} season
 */
function buildHubSeasonLiveLead(season) {
  const label = formatSeasonWindowLabel(season);
  return `<p class="landing-vision-lead landing-vision-lead--dev">
            <strong>Season live.</strong> ${label}
            <a href="/play/cedar-rapids/">Read the rules</a>
 ...
 * @param {string} [rel]
 */
export function applyResearchPageLaunchPatches(html, season, rel = "") {
  const label = formatSeasonWindowLabel(season);
  if (!label) {
    throw new Error("Season window dates required for research launch patch.");
  }

  const liveBlock = `<div class="research-live-banner hc-notice hc-notice--info" role="note">
          <p>
            <strong>Season live.</strong> ${label}
          </p>
          <p>
            <a href="/play/cedar-rapids/">Read the rules</a> before scanning game nodes. Resolver-backed play runs on live <code>/c/…</code> scan URLs — demo layouts on this page remain design reference.
          </p>
        </div>`;
  ...
  if (rel === "site/what-can-a-qr-do.html" && HUB_DEV_LEAD_RE.test(out)) {
    out = out.replace(HUB_DEV_LEAD_RE, buildHubSeasonLiveLead(season));
  }
 ...
 * @param {string} [rel]
 */
export function researchPageIsLaunchReady(html, rel = "") {
  const hasLiveBanner =
    html.includes("Season live.") && html.includes('href="/play/cedar-rapids/"');
  if (rel === "site/what-can-a-qr-do.html") {
    return (
      hasLiveBanner &&
      html.includes("landing-vision-lead--dev") &&
      html.includes("<strong>Season live.</strong>")
    );
  }
  return (
    hasLiveBanner &&
...
export function assessLaunchSurfacesApplied(season, surfaces) {
  ...
  for (const rel of RESEARCH_LAUNCH_PAGE_RELS) {
    ...
    if (!researchPageIsLaunchReady(html, rel)) {
