#!/usr/bin/env node
/**
 * Reset autonomous spine dev state (quorum, fragments, finale, witness scarcity).
 * Requires worker:dev + CITY_GAME_ENABLED=1 + seed file from city-game:seed-local.
 */
...
async function main() {
    ...
    const unsigned = withProtocolFields(
      {
        ...
        object_streams: patch.object_streams ?? current.object_streams,
  ...
  console.log("Resetting autonomous spine (local dev)…\n");

  await publish(SEASON_OBJECT_IDS.node_04, {
    public_state: "Seed clue live — share outward to evolve",
    game_meta: { collective_progress: 4, collective_target: 20, unlocked_by: [] },
  });

  await publish(SEASON_OBJECT_IDS.node_07, {
    public_state: "Locked until River Lantern quorum",
    game_meta: { unlocked_by: [], vouch_requires: ["node_10"], fragment_id: "czech_1" },
  });

  for (const nodeId of ["node_09", "node_11", "node_01"]) {
    await publish(SEASON_OBJECT_IDS[nodeId], {
      game_meta: { unlocked_by: [] },
    });
  }

  await publish(SEASON_OBJECT_IDS.node_13, {
    ...
    object_streams: [
      ...
      { id: "bulletin", class: "narrative", label: "Need", value: "0 / 3 fragments" },
      { id: "care", class: "care", label: "Site", value: "Clear" },
    ],
    game_meta: { unlocked_by: [] },
  });

  await publish(SEASON_OBJECT_IDS.node_10, {
    public_state: "Witness seal dormant — passes not yet issued",
    object_streams: [
      { id: "territory", class: "place", label: "Witness", value: "Library seal" },
      { id: "relay", class: "route", label: "Passes", value: "25 sunset passes remain" },
    ...
    game_meta: { scarcity_remaining: 25, vouch_active_for: [] },
  });

  console.log("\nSpine reset — replay with:");
