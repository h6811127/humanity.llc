...
import { clearWalletNetworkTruthForProfile } from "./device-wallet-network-truth.mjs?v=89";
import {
  hubNetworkChipStatusForProfile,
  shouldShowHubNetworkCheckingChip,
} from "./device-wallet-network-core.mjs?v=89";
import {
...
import {
  ...
  hubCardStatusLine,
  hubCardTitle,
} from "./device-hub-card-row-core.mjs?v=89";
import {
...
import { syncInboxBackdropForOpenHub } from "./device-sheet-backdrop-sync.mjs?v=89";
import {
...
function stewardScanHref(scanUrl) {
  return buildStewardScanPreviewHrefFromWindow(scanUrl);
}
    ...
    const revokedAlert = hubConfig.fetchNetworkStatus
      ? `<div class="hc-emphasis-card hc-emphasis-card--warn hub-card-status-alert" data-hub-searchable="${escapeHtml(CARD_DISABLED_SINCE_VISIT_SEARCH_SNIPPET)} network" hidden role="status">
          ...
          <div class="hc-emphasis-card__actions hub-card-status-alert-actions">
            ...
            <a class="hc-emphasis-card__cta hc-emphasis-card__cta--secondary hub-card-alert-view-scan" href="${escapeHtml(stewardScanHref(scan))}"${stewardScanAnchorAttrs()}>View scan</a>
          </div>
        </div>`
      : "";
...
function renderPinRows() {
  ...
  for (const pin of pins) {
    ...
    li.innerHTML = `
      <a href="${escapeHtml(stewardScanHref(pin.scan_url))}"${scanLinkAttrs}>
