...
- **Merch funnel MVP**: Scan → profile → QR customizer → Shopify → Printify — `docs/MERCH_FUNNEL_MVP.md` (§ Implementation priority stack). **Headless wiring:** `docs/MERCH_HEADLESS_COMMERCE.md`. Tier 0: `/shop/` · Tier 1: `/shop/customize/` · scan CTA `hc_ref=scan_customize` · artifact intent API · Worker route `humanity.llc/v1/*`. Engineering gate: `npm run merch-funnel:verify-exit` · CI fast gate: `merch-funnel:verify-exit:fast` · operator: `merch-funnel:verify-config -- --require-checkout` · E2E: `e2e:merch-funnel` · physical print QA: `docs/MERCH_PHYSICAL_QA_RUNBOOK.md` · `npm run worker:test:merch-print-qa`. After `site/scan-pass.css` changes: `npm run worker:bundle-scan`. **Next gate:** operator Tier 1 close-out (M5 ✅ 2026-05-27).
