...
- **Sad-path coverage:** `docs/SAD_PATH_COVERAGE_AND_BACKLOG.md` — unhappy-path inventory, agent vs human validation, prioritized backlog (live proof Slice D: H-09/H-10 in `scan-html.ts`).
...
- **Product language:** Default UI uses Layer 2 outcome copy; protocol terms stay in Advanced, Help, and engineer docs. Policy: `docs/PRODUCT_LANGUAGE_STRATEGY.md` · terminology map: `docs/OWNERSHIP_AND_CONTROL_MODEL.md`. Before changing user-visible strings, check § Checklist for new copy.
...
- **PWA install (device shell):** `docs/PWA_INSTALL.md` — steward-only Add to Home Screen on `/`, `/wallet/`, `/created/`; **never** scan or `/create/`. Load `pwa-install.mjs` and `pwa-standalone-refresh.mjs` **lazily** after status bootstrap — do not static-import from `device-status.mjs` unless added to `DEVICE_STATUS_SHELL_JS_FILES` in the same PR. **No shell-caching service worker** (cache skew risk). **Standalone refresh:** Phase 6 shipped (resume soft refresh); Phases 7–8 PTR + stale shell nudge — see § Standalone refresh & resume. Tests: `npm run worker:test:pwa-install` · manual **P1-PWA** / **P1-PWA-R** in `docs/DEVICE_OS_QA.md`.
