...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c. **Hosted tier:** **M8 code complete in staging** behind `HOSTED_STEWARD_ENABLED` (local dev `wrangler.toml`); production blocked on **G0**. See `docs/HOSTED_TIER_IMPLEMENTATION_EPICS.md`. Tests: `npm run worker:test:steward-hosted` · `npm run e2e:steward-hosted`. Ops: `docs/HOSTED_STEWARD_OPS_RUNBOOK.md` · E6.1 `docs/HOSTED_STEWARD_CF_DASHBOARD.md` · E6.2 CI `.github/workflows/steward-ops-daily.yml`. After migrate: `0012_steward_hosted.sql`, `0013_steward_billing.sql`.
