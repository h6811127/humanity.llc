...
- **Merch funnel MVP**: Scan → profile → QR customizer → Shopify → Printify — `docs/MERCH_FUNNEL_MVP.md`. Shop hub: `/shop/` · Tier 0 founding: `/shop/founding/` · Tier 1 customizer: `/shop/customize/` · scan CTA `hc_ref=scan_customize` · artifact intent API. E2E: `npm run e2e:merch-funnel`. After `site/scan-pass.css` changes: `npm run worker:bundle-scan`.
