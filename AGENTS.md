...
- **Pages build stamp (debug):** `docs/SITE_BUILD_VERSIONING.md` — `npm run site:build-meta` before Pages deploy; bootstrap logs `[humanity] site build …` on shell pages; hub stamp when `localStorage.hc_debug === "1"` or `?hc_debug=1`. After meta/bootstrap/hub-stamp changes: `npm run worker:test -- worker/tests/site-build-meta.test.ts worker/tests/device-status-shell-modules.test.ts`.
