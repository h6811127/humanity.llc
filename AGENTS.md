...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c (caps, leader tab, ETag/304, etc.). **Paid tier (planning):** M1 `PAID_TIER_AND_HOSTED_OPERATOR_PLAN.md`, M2 `HOSTED_TIER_ENTITLEMENTS_AND_METERING.md`, M3 `HOSTED_TIER_PUSH_ARCHITECTURE_RFC.md`, M4 `HOSTED_TIER_PRICING_AND_SLA.md` — no billing/push code until M5–M6 + M4 governance sign-off. Wallet status: hub-expand refresh + 60s visibility debounce.
