...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Live-control Phases 1–3: scoped polling (hub/inbox/wallet), round-robin one GET per tick, 30s/5s intervals, no timer when resolver health ≠ `ok`. Wallet status: hub-expand refresh + 60s visibility debounce.
