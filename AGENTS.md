...
3. Run `npm run verify:desk:fast` (every change); `npm run verify:desk` before merge. Surface extras: workstream doc · city game: `npm run verify:city-game` · public network player flow: `npm run verify:public-network-player-flow:fast` (shells) / `npm run verify:public-network-player-flow` (+ e2e) / `npm run verify:public-network-player-flow:production` (post-deploy) · human field walk: `npm run player-flow:field-kit` · PD sign-off: `npm run player-flow:sign-off`. **Landing `/`:** discovery-first hero **“Check what's true right now…”** + places-first **`#landing-places`** (P5a–b) — never restore “Live state on real objects,” hero Create CTA, retired **`#launch-doors`**, or boards-primary **Public live boards** H2; run `npm run verify:landing` (contract: `site/js/landing-copy-contract.mjs`).
