...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9: scoped polling, round-robin, watch default off, **400 auto-poll/day** cap, **leader tab** + BroadcastChannel snapshot, large-wallet (≥10) hint + narrowed poll set, network `maxParallel` cap, **ETag/304** on status + pending live-control GETs (`resolver-conditional-fetch-core.mjs`). Next: Phase 8c. Wallet status: hub-expand refresh + 60s visibility debounce.
