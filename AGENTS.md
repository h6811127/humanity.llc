...
- **AI L3 P1 (explain snapshot)**: Opt-in plain-language summary of signed `public_snapshot` — `docs/AI_FEATURE_DEVELOPMENT.md` · `docs/AI_L3_EXPLAIN_SNAPSHOT.md`. Worker: `POST /.well-known/hc/v1/ai/explain-snapshot` · `[ai]` binding in `worker/wrangler.toml`. Scan: `site/js/scan-ai-explain.mjs` on status plate / live object when streams present. After changes: `npm run worker:test -- worker/tests/ai-explain-core.test.ts worker/tests/ai-explain-snapshot.test.ts` · `npm run worker:bundle-scan`.
