...
- **Merch funnel MVP**: Scan → profile → QR customizer → Shopify → Printify — `docs/MERCH_FUNNEL_MVP.md` (§ Implementation priority stack). Tier 0: `/shop/` · Tier 1: `/shop/customize/` · scan CTA `hc_ref=scan_customize` · artifact intent API. After `site/scan-pass.css` changes: `npm run worker:bundle-scan`.
