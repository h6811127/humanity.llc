...
- **Sad-path coverage:** `docs/SAD_PATH_COVERAGE_AND_BACKLOG.md` — unhappy-path inventory, agent vs human validation, prioritized backlog; live proof E2E: `npm run e2e:live-control-loop`; printed QR H-12 pre-flight: `live-control:printed-qa:preflight` → `production-smoke` → `two-device-loop`.
...
- **PWA vs browser session mismatch (P2-2):** `docs/SAFARI_KEYS_WIPE_INVESTIGATION.md` — `hc_last_signing_shell_mode` + restore CTA in standalone when signing last happened in Safari (R5). Tests: `npm run worker:test:pwa-session-mismatch`.
