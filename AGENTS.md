...
**Red outline ring on dot** = `device-status.mjs` import graph failed (`data-device-status-error`). Load-error **coach card** auto-shows (`device-status-load-error.mjs`); dot tap toggles it — not the hub. See `docs/STATUS_DOT_LOAD_FAILURE_POSTMORTEM.md`. Never merge a new `./device-*.mjs` import on the status graph without the file in the same PR; add the filename to `DEVICE_STATUS_SHELL_JS_FILES` in `site/js/device-status-shell-modules.mjs`, bump `DEVICE_SHELL_ASSET_VERSION` on shell HTML bootstrap and on every `./peer.mjs?v=N` import between manifest peers. Run `npm run worker:test -- worker/tests/device-status-shell-modules.test.ts`. **Lazy subgraphs (Shell P2):** `device-inbox-sheet-loader.mjs`, `device-inbox-loader.mjs`, and `device-browser-notifications-loader.mjs` — do not static-import full inbox/sheet/notifications from `device-status.mjs` or `device-chrome-refresh.mjs`. Tests: `npm run worker:test -- worker/tests/device-status-lazy-inbox.test.ts` · load-error: `npm run worker:test -- worker/tests/device-status-load-error.test.ts` · E2E `status load error shows coach card`.
