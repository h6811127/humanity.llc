...
- **Product language:** Default UI uses Layer 2 outcome copy; protocol terms stay in Advanced, Help, and engineer docs. Policy: `docs/PRODUCT_LANGUAGE_STRATEGY.md` · terminology map: `docs/OWNERSHIP_AND_CONTROL_MODEL.md` · shared strings: `site/js/device-ownership-copy-core.mjs`. Before changing user-visible strings, check § Checklist for new copy.
...
- **PWA install (device shell):** `docs/PWA_INSTALL.md` — steward-only Add to Home Screen on `/`, `/wallet/`, `/created/`; **never** scan or `/create/`. Load `pwa-install.mjs` and `pwa-standalone-refresh.mjs` **lazily** after status bootstrap. **Standalone refresh:** Phases 6–7 shipped (resume soft refresh + PTR on `/` and `/wallet/`); Phase 8 stale shell nudge pending. Tests: `npm run worker:test:pwa-install` · `npm run e2e:pwa-install` · manual **P1-PWA** / **P1-PWA-R** in `docs/DEVICE_OS_QA.md`.
