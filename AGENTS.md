...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c. **Resolver tab sync (Phase 1a):** `docs/DEVICE_TAB_RESOLVER_SYNC.md` — `hc-resolver-sync` BC; followers skip duplicate status GETs within 60s. After hub network changes: `npm run worker:test -- worker/tests/device-resolver-sync.test.ts` · manual **P1-1** in `docs/DEVICE_OS_QA.md`. **Hosted tier:** **M8 complete**; **G0 signed** (Governance + Ops; Legal pending G7). Production rollout: [`docs/HOSTED_TIER_IMPLEMENTATION_EPICS.md`](docs/HOSTED_TIER_IMPLEMENTATION_EPICS.md) § Production rollout. Tests: `npm run verify:hosted-g0` · `npm run e2e:steward-hosted`. Ops: `docs/HOSTED_STEWARD_OPS_RUNBOOK.md` · E6.1 `docs/HOSTED_STEWARD_CF_DASHBOARD.md` · E6.2 CI `.github/workflows/steward-ops-daily.yml`. After migrate: `0012_steward_hosted.sql`, `0013_steward_billing.sql`.
