...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c (caps, leader tab, ETag/304, etc.). **Paid tier (planning):** M1–M7 complete under `docs/PAID_TIER_AND_HOSTED_OPERATOR_PLAN.md` — no billing/push code until M4 governance sign-off + M8 epics. Wallet status: hub-expand refresh + 60s visibility debounce.
