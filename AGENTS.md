...
- **PWA install (device shell):** `docs/PWA_INSTALL.md` — steward-only Add to Home Screen on `/`, `/wallet/`, `/created/`; **never** scan or `/create/`. Load `pwa-install.mjs` **lazily** after status bootstrap — do not static-import from `device-status.mjs` unless added to `DEVICE_STATUS_SHELL_JS_FILES` in the same PR. **No shell-caching service worker** (cache skew risk). **Standalone refresh (Phases 6–8, H-007):** soft refresh on resume + pull-to-refresh on `/` and `/wallet/` — lazy `pwa-standalone-refresh.mjs`; see § Standalone refresh & resume. Tests: `npm run worker:test:pwa-install` · `worker/tests/pwa-standalone-refresh-core.test.ts` (when shipped) · manual **P1-PWA** / **P1-PWA-R** in `docs/DEVICE_OS_QA.md`.
