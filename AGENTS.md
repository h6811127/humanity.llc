...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c. **Hosted tier:** planning M1–M8 complete; **E1** Worker API behind `HOSTED_STEWARD_ENABLED` (local dev `wrangler.toml`); client E2+ not started. See `docs/HOSTED_TIER_IMPLEMENTATION_EPICS.md`.
