...
- **Merch funnel MVP**: Scan → profile → QR customizer → Shopify → Printify — `docs/MERCH_FUNNEL_MVP.md` (§ Implementation priority stack). **Visual choreography:** `docs/MERCH_VISUAL_CHOREOGRAPHY.md` (owner delight on customize/created; calm notary on scan). **Headless wiring:** `docs/MERCH_HEADLESS_COMMERCE.md`. **Sad paths:** `docs/MERCH_CHECKOUT_SAD_PATH_MATRIX.md` · E2E `e2e:merch-checkout-sad-path` (M1–M2) + `e2e:merch-funnel`. Tier 0: `/shop/` · Tier 1: `/shop/customize/` · scan CTA `hc_ref=scan_customize` · artifact intent API · Worker route `humanity.llc/v1/*`. Engineering gate: `npm run merch-funnel:verify-exit` · close-out rollout: `merch-funnel:rollout:step1` → `step6` · operator: `merch-funnel:verify-config` · physical print QA: `docs/MERCH_PHYSICAL_QA_RUNBOOK.md` · `npm run worker:test:merch-print-qa`. After `site/scan-pass.css` changes: `npm run worker:bundle-scan`. **Next product gate:** M5 stranger runbook (Priority 2) before live Tier 1 checkout.
...
- **Sad-path coverage:** `docs/SAD_PATH_COVERAGE_AND_BACKLOG.md` — unhappy-path inventory, agent vs human validation, prioritized backlog; live proof E2E: `npm run e2e:live-control-loop`; printed QR H-12: `live-control:printed-qa:desk-gate` → human § A–C → `live-control:printed-qa:sign-off`.
- **Ownership restore UX:** `docs/OWNERSHIP_RESTORE_UX_PLAN.md` — view-only `/created/` + hub import always visible; engineering gate: `npm run ownership-restore:verify`; manual **P1-RESTORE** / **P1-HE** in `docs/DEVICE_OS_QA.md`.
...
- **Product language:** Default UI uses Layer 2 outcome copy; protocol terms stay in Advanced, Help, and engineer docs. Policy: `docs/PRODUCT_LANGUAGE_STRATEGY.md` · terminology map: `docs/OWNERSHIP_AND_CONTROL_MODEL.md` · shared strings: `site/js/device-ownership-copy-core.mjs`. Comprehension copy guards (D9): `npm run worker:test:comprehension`. Before changing user-visible strings, check § Checklist for new copy.
- **Ownership restore (view mode / hub):** Cross-device = **recovery code** (primary) or encrypted backup (advanced). View-only `/created/` must expose restore (planned) — `docs/OWNERSHIP_RESTORE_UX_PLAN.md`. Hub restore section always visible (Phase 1). Setup hard gate before “You're live” (Phase 2).
...
- **Card disabled since visit:** Banner, hub `#device-hub-card-disabled-group`, and inbox badge must use **resolver-confirmed** poll maps only — never `sessionStorage.hc_wallet_network_cache` alone. Incident **closed** (no Slice 9); see `docs/CARD_DISABLED_SINCE_VISIT_FALSE_POSITIVE_INVESTIGATION.md`. After hub/inbox network changes: `npm run worker:test:card-disabled-since-visit` · `npm run e2e:card-disabled-since-visit` · desk gate: `npm run card-disabled-since-visit:desk-gate` (P0b-1 step 2) · manual **P1-P0b-1** in `docs/DEVICE_OS_QA.md`.
...
- **PWA install (device shell):** `docs/PWA_INSTALL.md` — Phases 1–10 shipped (install + standalone resume soft refresh + PTR + stale shell nudge + supplementary affordances + browser-vs-PWA shortcut visibility). Lazy-load `pwa-install.mjs` and `pwa-standalone-refresh.mjs` after status bootstrap. Tab-native shortcuts hidden in standalone via `pwa-browser-tab-shortcuts.mjs` on `/`. Tests: `npm run worker:test:pwa-install` · `npm run e2e:pwa-install` · manual **P1-PWA** / **P1-PWA-R** in `docs/DEVICE_OS_QA.md`.
- **Safari ITP storage notice (P2-1):** `docs/SAFARI_KEYS_WIPE_INVESTIGATION.md` — iOS shell card for ~7-day eviction + Home Screen timer reset (R4). Lazy-load `safari-itp-storage-notice.mjs` after status bootstrap. Tests: `npm run worker:test:safari-itp-notice`.
- **Safari persist-denied notice (RC-2):** `docs/HUB_CARD_DISAPPEARED_SAFARI_INVESTIGATION.md` — iOS shell warn card when `navigator.storage.persist()` returns false (`hc_storage_persist_requested_v1 = "0"`). Lazy-load `safari-storage-persist-denied-notice.mjs` after status bootstrap; `device-storage-persist.mjs` fires `hc-storage-persist-settled`. Tests: `npm run worker:test:safari-persist-denied-notice` · manual **P2-RC2** in `docs/DEVICE_OS_QA.md`.
- **Canonical origin (RC-13):** `docs/HUB_CARD_DISAPPEARED_SAFARI_INVESTIGATION.md` — `www.humanity.llc` → apex redirect on shell pages + Worker 301; debug hub origin line when `hc_debug=1`. Tests: `npm run worker:test:canonical-origin`.
- **Hub card disappeared (closed — monitoring):** `docs/HUB_CARD_DISAPPEARED_SAFARI_INVESTIGATION.md` — RC-1–RC-16 shipped; steward diagnostic via Web Inspector snippet or hub **Copy build info** with `hc_debug=1`. Engineering gate: `npm run hub-card-disappeared:verify` · fast CI: `hub-card-disappeared:verify:fast` · manual **P2-RC-MON** in `docs/DEVICE_OS_QA.md`.
- **Safari PWA session mismatch (P2-2):** `device-pwa-session-mismatch-core.mjs` — `hc_last_signing_shell_mode` when tab gains keys; hub custody · `/wallet/` tab hint · scan actor band. Tests: `npm run worker:test:pwa-session-mismatch`.
- **Steward scan handoff / PWA vouch (S1–S7):** `docs/STEWARD_SCAN_HANDOFF_AND_PWA_VOUCH.md` — S1–S6 handoff paths · **S7 dual-QR on `/created/`** (`steward-dual-qr-core.mjs`). Gate: `npm run steward-scan-handoff:verify` · manual **P1-PWA-V** in `docs/DEVICE_OS_QA.md`.
- **Safari keys WebKit E2E (P2-3 / P0-1):** `e2e/safari-keys-persistence.spec.ts` — P0-1 scan quiet rehydrate · S2/S3 matrix. Run: `npm run e2e:safari-keys-persistence` (after `e2e:install`).
- **Scan dot R9 E2E (P0-5):** `e2e/scan-page-dot.spec.ts` — wallet saved, tab empty → `ok:none` + restore glance. Run: `npm run e2e:scan-page-dot`.
- **Safari hub summary key guard (P3-3):** `device-wallet-summary-core.mjs` — allowlisted `hc_wallet_summary` rows + persist tripwire. Tests: `npm run worker:test -- worker/tests/device-wallet-summary-core.test.ts worker/tests/device-wallet-meta.test.ts`.
- **Parallel workstreams (Safari · ownership-restore · H-12):** [`docs/PRODUCT_WORKSTREAM_COORDINATION.md`](docs/PRODUCT_WORKSTREAM_COORDINATION.md) — active PRs, open items, regression index, agent handoff log.
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c. **Resolver tab sync (1a–1b + landing toggle):** `docs/DEVICE_TAB_RESOLVER_SYNC.md` — `hc-resolver-sync` BC (network 60s, health 30s); opt out via homepage **Share network checks** or `hc_resolver_sync_tabs=0`. After hub network changes: `npm run worker:test -- worker/tests/device-resolver-sync.test.ts` · `npm run e2e:device-resolver-sync` · manual **P1-1** in `docs/DEVICE_OS_QA.md`. **Hosted tier:** **M8 complete**; **G0 signed** (Governance + Ops; Legal pending G7). Production rollout: [`docs/HOSTED_TIER_IMPLEMENTATION_EPICS.md`](docs/HOSTED_TIER_IMPLEMENTATION_EPICS.md) § Production rollout — `hosted:rollout:step1` → `step6`. Final regression: `hosted:rollout:step6 -- --verify`. Ops: `docs/HOSTED_STEWARD_OPS_RUNBOOK.md` · E6.1 `docs/HOSTED_STEWARD_CF_DASHBOARD.md` · E6.2 CI `.github/workflows/steward-ops-daily.yml`. Migrations: `0012_steward_hosted.sql`, `0013_steward_billing.sql`.

- **Shell page load flash (RC-1–RC-4):** `docs/SHELL_PAGE_LOAD_CONTENT_FLASH_INVESTIGATION.md` — `data-boot` / `data-dot-boot` gates hide personalized rows and defer dot paint until health + session settle; hub chips show **checking** until `isResolverConfirmedProfile`. Tests: `npm run worker:test:shell-boot` · bump `DEVICE_SHELL_ASSET_VERSION` in `device-status-shell-modules.mjs` when adding shell modules.
...
**Red outline ring on dot** = `device-status.mjs` import graph failed (`data-device-status-error`). Load-error **coach card** auto-shows (`device-status-load-error.mjs`); dot tap toggles it — not the hub. See `docs/STATUS_DOT_LOAD_FAILURE_POSTMORTEM.md`. Never merge a new `./device-*.mjs` import on the status graph without the file in the same PR; add the filename to `DEVICE_STATUS_SHELL_JS_FILES` in `site/js/device-status-shell-modules.mjs`, bump `DEVICE_SHELL_ASSET_VERSION` on shell HTML bootstrap and on every `./peer.mjs?v=N` import between manifest peers. Run `npm run worker:test -- worker/tests/device-status-shell-modules.test.ts`. **Lazy subgraphs (Shell P2):** `device-inbox-sheet-loader.mjs`, `device-hub-sheet-loader.mjs`, `device-inbox-loader.mjs`, and `device-browser-notifications-loader.mjs` — do not static-import full inbox/hub-sheet/notifications from `device-status.mjs` or `device-chrome-refresh.mjs`. Tests: `npm run worker:test -- worker/tests/device-status-lazy-inbox.test.ts` · load-error: `npm run worker:test -- worker/tests/device-status-load-error.test.ts` · E2E `status load error shows coach card`.
...
1. **Module graph** — `device-status-bootstrap.mjs` → inner loads `device-status-core.mjs` then `device-status.mjs`. **Partial** failure (core OK, status failed): `data-device-status-partial`, hub opens, no coach hijack. **Total** failure: `data-device-status-error` + load-error coach. New imports must ship in the same deploy and stay listed in `site/js/device-status-shell-modules.mjs` (Vitest + `e2e/device-status-dot.spec.ts`).
