...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c. **Paid tier (planning M1–M8 complete):** start implementation at `docs/HOSTED_TIER_IMPLEMENTATION_EPICS.md` **E1** only after M4 governance sign-off on `HOSTED_TIER_PRICING_AND_SLA.md`. Wallet status: hub-expand refresh + 60s visibility debounce.
