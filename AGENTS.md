...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c (caps, leader tab, ETag/304, etc.). **Paid tier (planning):** M1–M5 docs under `docs/PAID_TIER_AND_HOSTED_OPERATOR_PLAN.md` (through launch language) — no billing/push code until M6 + M4 governance sign-off. Wallet status: hub-expand refresh + 60s visibility debounce.
