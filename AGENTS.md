...
- **PWA install (device shell):** `docs/PWA_INSTALL.md` — Phases 1–9 shipped (install + standalone resume soft refresh + PTR + stale shell nudge + supplementary affordances). Lazy-load `pwa-install.mjs` and `pwa-standalone-refresh.mjs` after status bootstrap. Tests: `npm run worker:test:pwa-install` · `npm run e2e:pwa-install` · manual **P1-PWA** / **P1-PWA-R** in `docs/DEVICE_OS_QA.md`.
...
- **Steward scan handoff / PWA vouch (S1–S5):** `docs/STEWARD_SCAN_HANDOFF_AND_PWA_VOUCH.md` — camera→Safari vs PWA keys; S1 clipboard handoff · S2 hub recovery import · S3 in-app hub QR scanner · S4 product guidance · **S5 `?hc_steward=1` Safari handoff-first** (`scan-pwa-camera-handoff-core.mjs`). Tests: `npm run worker:test:steward-scan-handoff` · manual **P1-PWA-V** in `docs/DEVICE_OS_QA.md`.
