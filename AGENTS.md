...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c. **Hosted tier:** **E1–E6 staging** behind `HOSTED_STEWARD_ENABLED` (local dev `wrangler.toml`). See `docs/HOSTED_TIER_IMPLEMENTATION_EPICS.md`. Tests: `npm run worker:test:steward-entitlements` · `npm run worker:test:steward-push` · `npm run worker:test:steward-ops` · `npm run e2e:hosted-tier` · `npm run e2e:hosted-tier-push`. Ops daily: `npm run worker:check-steward-ops` (requires `OPERATOR_AUDIT_TOKEN`). After migrate: `0012_steward_hosted.sql`, `0013_steward_billing.sql`.
