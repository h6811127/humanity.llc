...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c. **Hosted tier:** **E1** Worker API + **E5** billing webhooks behind `HOSTED_STEWARD_ENABLED` (local dev `wrangler.toml`); client **E2** entitlements probe wired. See `docs/HOSTED_TIER_IMPLEMENTATION_EPICS.md`. After migrate: `0012_steward_hosted.sql`, `0013_steward_billing.sql`.
