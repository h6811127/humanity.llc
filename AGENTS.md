...
- **Merch funnel MVP**: Scan → profile → QR customizer → Shopify → Printify — `docs/MERCH_FUNNEL_MVP.md`. **Headless wiring:** `docs/MERCH_HEADLESS_COMMERCE.md` (Printify = factory, Shopify = checkout, humanity.llc = store). Tier 0: `/shop/` · Tier 1 customizer: `/shop/customize/` · artifact intent API. **Close-out rollout:** `merch-funnel:rollout:step1` → `step6` · `verify:merch-funnel` — `docs/MERCH_HEADLESS_COMMERCE.md` § Production rollout commands. Physical print QA: `docs/MERCH_PHYSICAL_QA_RUNBOOK.md` · `npm run worker:test:merch-print-qa`.
