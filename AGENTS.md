...
- **PWA install (device shell):** `docs/PWA_INSTALL.md` — Phases 1–8 shipped (install + standalone resume soft refresh + PTR + stale shell nudge). Lazy-load `pwa-install.mjs` and `pwa-standalone-refresh.mjs` after status bootstrap. Tests: `npm run worker:test:pwa-install` · `npm run e2e:pwa-install` · manual **P1-PWA** / **P1-PWA-R** in `docs/DEVICE_OS_QA.md`.
