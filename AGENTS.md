...
- **Worker request budget / polling:** Read `docs/DEVICE_OS_REQUEST_BUDGET.md` before shell network I/O. Shipped Phases 1–9 + 8c (caps, leader tab, ETag/304, etc.). **Paid tier (planning):** `docs/PAID_TIER_AND_HOSTED_OPERATOR_PLAN.md` (M1), `docs/HOSTED_TIER_ENTITLEMENTS_AND_METERING.md` (M2), `docs/HOSTED_TIER_PUSH_ARCHITECTURE_RFC.md` (M3) — no push/entitlement code until M4–M6 + governance. Wallet status: hub-expand refresh + 60s visibility debounce.
