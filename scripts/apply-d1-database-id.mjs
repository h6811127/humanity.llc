 ...
 * - Otherwise prints how to fix Cloudflare error 10021 and exits 1.
 */
...
console.error(`
apply-d1-database-id: D1 binding has no valid database_id (Cloudflare API error 10021).
...
Do one of the following:
  ...
  2) For Workers Builds / CI: set environment variable D1_DATABASE_ID to that UUID
     and use "npm run deploy" (not plain "wrangler deploy") so the predeploy hook runs.
